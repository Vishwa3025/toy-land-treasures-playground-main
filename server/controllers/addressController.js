...
const getUserAddress = async (req, res) => {
    try {
        ...
        const userId = req.params.id; // Use the ID from the URL

        const address = await Address.findOne({ where: { user_id: userId } });


        res.json({ exists: !!address, address });
    } catch (error) {
...
const createOrUpdateAddress = async (req, res) => {
    try {
        ...
        if (existingAddress) {
            ...
            await existingAddress.update({ full_name, address_line1, address_line2, city, state, postal_code, country, phone });
            return res.status(200).json({ message: "Address updated", address: existingAddress });
        } else {
            // Create new address
            const newAddress = await Address.create({ user_id: userId, full_name, address_line1, address_line2, city, state, postal_code, country, phone });
